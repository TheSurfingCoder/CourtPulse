...
const Court_1 = require("../../../src/models/Court");
...
describe('CourtModel', () => {
    ...
    describe('findAll', () => {
        it('should return all courts', async () => {
            ...
            const result = await Court_1.CourtModel.findByType('basketball');
            ...
            expect(mockPool.query).toHaveBeenCalledWith(expect.stringContaining('SELECT'), ['basketball']);
        });
        it('should handle database errors', async () => {
            ...
            await expect(Court_1.CourtModel.findByType('basketball')).rejects.toThrow('Database connection failed');
        });
    });
    describe('findById', () => {
        it('should return court by id', async () => {
            ...
            const result = await Court_1.CourtModel.findById(1);
        ...
        it('should return null for non-existent court', async () => {
            ...
            const result = await Court_1.CourtModel.findById(999);
    ...
    describe('create', () => {
        it('should create a new court', async () => {
            ...
            const result = await Court_1.CourtModel.create(courtData);
            ...
            expect(mockPool.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO courts'), ['New Court', 'tennis', -73.9851, 40.7589, 'clay', true]);
        });
    });
    describe('findByType', () => {
        it('should return courts by type', async () => {
            ...
            expect(result).toEqual(mockCourts);
            // Assert: Check the database was called with correct query and parameters
            expect(mockPool.query).toHaveBeenCalledWith(expect.stringContaining('WHERE sport = $1'), ['basketball']);
        });
        it('should return empty array for non-existent type', async () => {
            // Arrange: Mock database to return empty results
            mockPool.query.mockResolvedValue({ rows: [] });
            // Act: Try to find courts of non-existent type
            const result = await Court_1.CourtModel.findByType('nonexistent');
    ...
    describe('update', () => {
        it('should update court with valid data', async () => {
            ...
            const result = await Court_1.CourtModel.update(1, updateData);
            ...
            expect(mockPool.query).toHaveBeenCalledWith(expect.stringContaining('UPDATE courts'), expect.arrayContaining(['Updated Court', 'tennis', 'clay', 1]));
        });
        ...
        it('should return null when court not found', async () => {
            ...
            const result = await Court_1.CourtModel.update(999, { name: 'Updated' });
            // Assert: Should return null
            expect(result).toBeNull();
        });
    });
    describe('delete', () => {
        it('should delete court successfully', async () => {
            ...
            const result = await Court_1.CourtModel.delete(1);
            // Assert: Should return true
            expect(result).toBe(true);
            // Assert: Check the database was called with DELETE query
            expect(mockPool.query).toHaveBeenCalledWith('DELETE FROM courts WHERE id = $1', [1]);
        });
        it('should return false when court not found', async () => {
            ...
            const result = await Court_1.CourtModel.delete(999);
