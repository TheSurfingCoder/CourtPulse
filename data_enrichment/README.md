...
• `populate_cluster_metadata.py` - Database-side clustering: groups courts by `facility_name` using SQL, assigns shared `cluster_id` (UUID) in database, transfers to `courts` table
• `add_individual_court_names.py` - Adds sequential names ("Court 1", "Court 2") to clustered courts
...
3. **Clustering** - Group courts by facility_name
   - **When**: Post-processing step after data is in `osm_courts_temp` (standalone script)
   - **How**: `populate_cluster_metadata.py` uses SQL to group by `facility_name` in database, assigns shared `cluster_id` (UUID) via `gen_random_uuid()`, then transfers to `courts` table
   - **Location**: Database-side (SQL) - efficient single-query operation, no in-memory processing
   - **Result**: Courts with same `facility_name` share same `cluster_id` for frontend grouping
...
5. **Naming** - Assign individual court names within clusters
   - **Flow**: `add_individual_court_names.py` groups courts by `cluster_id`, assigns sequential names ("Court 1", "Court 2", etc.) to clusters with multiple courts
   - **Ordering**: Courts ordered by `id` for consistent naming within each cluster
   - **Database → Frontend Mapping**:
     - Frontend `name` = `COALESCE(individual_court_name, enriched_name, fallback_name, 'Unknown Court')`
     - Frontend `cluster_group_name` = `COALESCE(photon_name, enriched_name, fallback_name, NULL)` (should map to `facility_name` in future)
   - **Frontend Display**:
   ...
   - **Examples**:
     ...
     - **Multi-court cluster** (Golden Gate Park, 3 basketball courts):
       - Court 1: `name = "Court 1"`, `cluster_group_name = "Golden Gate Park"`
       - Court 2: `name = "Court 2"`, `cluster_group_name = "Golden Gate Park"`
       - Court 3: `name = "Court 3"`, `cluster_group_name = "Golden Gate Park"`
